...
前往 [Releases](https://github.com/doubly-yi/IMEmory/releases) 下载最新版压缩包，解压后把 `输入定格.app` 拖进**应用程序**文件夹。

> ⚠️ 预编译包用个人开发者证书签名、未做 Apple 公证，首次打开会被系统拦下并提示「Apple 无法验证…是否包含恶意软件」。这是正常的，按下面任一方式放行即可（介意的话也可以直接[从源码构建](#编译安装)）。

**放行方法一(系统设置，推荐):**

1. 双击 `输入定格.app`，在弹窗里点**完成**(先别点「移到废纸篓」)。
2. 打开**系统设置 › 隐私与安全性**，下拉到「安全性」，会看到一行「已阻止"输入定格"以保护 Mac」，点右侧**仍要打开**。
3. 再次确认，按提示用 Touch ID 或密码验证，之后即可正常启动。

**放行方法二(终端一行命令):**

```bash
xattr -dr com.apple.quarantine /Applications/输入定格.app
```

放行后首次运行，还要到**系统设置 › 隐私与安全性**开启两个权限：**屏幕录制**(识别输入法中/英 HUD)和**辅助功能**(合成 Shift 切换、检测文本焦点)。
